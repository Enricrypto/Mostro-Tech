...
type Token = "MLUN" | "MATL" | "MLIZ"

type OHLC = { x: Date; y: [number, number, number, number] }
...
export const Chart = ({ token }: ChartProps) => {
  ...
  // Select timeframe-specific data
  const tokenData =
    token === "MLUN"
      ? mockPriceDataMLUN[selectedTimeframe]
      : token === "MATL"
      ? mockPriceDataMATL[selectedTimeframe]
      : mockPriceDataMLIZ[selectedTimeframe]

  // Full data for current price
  const allData =
    token === "MLUN"
      ? mockPriceDataMLUN.All
      : token === "MATL"
      ? mockPriceDataMATL.All
      : mockPriceDataMLIZ.All

  const lastPrice = allData[allData.length - 1]

  // Build OHLC data, ensuring last candle = current price
  const ohlcData: OHLC[] = tokenData.map((price, i) => {
    const open = i === 0 ? tokenData[0] : tokenData[i - 1]
    const close = i === tokenData.length - 1 ? lastPrice : price
    ...
    switch (selectedTimeframe) {
      case "1h":
        timestamp = new Date(
          now.getTime() - (tokenData.length - 1 - i) * 60 * 1000
        )
        break
      case "24h":
        timestamp = new Date(
          now.getTime() - (tokenData.length - 1 - i) * 60 * 60 * 1000
        )
        break
      case "7d":
      case "30d":
      case "90d":
        timestamp = new Date(
          now.getTime() - (tokenData.length - 1 - i) * 24 * 60 * 60 * 1000
        )
        break
      case "1y":
      case "All":
        const monthsOld = tokenData.length
  ...
  const formattedCurrentPrice = lastClosePrice.toFixed(2)
