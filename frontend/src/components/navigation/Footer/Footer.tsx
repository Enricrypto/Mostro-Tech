...
export function Footer() {
  ...
  return (
    ...
    <div
      className={cn(
        "w-full flex flex-col gap-16 pt-10",
        "bg-[#121B2B] border-t-2 border-[#DCFD63] backdrop-blur-xs"
      )}
    >
      {/* Top Section */}
      <footer className={cn("flex justify-between w-full h-[184px] px-16")}>
        {/* Logo */}
        ...
        <div className='flex flex-col justify-between w-[407px] h-[184px]'>
          ...
          <div className='flex justify-between w-full h-8 gap-2'>
            {socialButtons.map((btn) => (
              <PillButton
                key={btn.label}
                themeVariant='primary'
                className={cn(
                  "flex items-center justify-center gap-2 whitespace-nowrap overflow-hidden text-ellipsis",
                  "w-[125px] h-8",
                  "rounded-[34px]",
                  "px-3 py-2",
                  "border border-[#2D3953]",
                  "bg-[#DCFD63]",
                  "text-black font-body font-normal text-[14px] leading-4"
                )}
              >
                {btn.icon} {btn.label}
              </PillButton>
      ...
      <div className='w-full flex justify-center bg-[#71BEFB]'>
        <div className='w-full flex justify-between items-center  px-6 py-2'>
          ...
          {/* Right buttons */}
          <div className='flex gap-2 w-md h-10 justify-end'>
