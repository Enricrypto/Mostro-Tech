...
export function Alert({
  ...
  const VARIANT_MAP: Record<Variant, VariantClasses> = {
    default: {
      background: "bg-[var(--color-alert-night)] border border-[var(--color-alert-blue)]",
    ...
    alert: {
      background: "bg-[var(--color-alert-night)] border border-[var(--color-alert-red)]",
    ...
    confirm: {
      background: "bg-[var(--color-alert-night)] border border-[var(--color-alert-green)]",
    ...
    alertIcon: {
      ...
      primary_button: "button-alert-red-primary",
      secondary_button: "button-alert-red-secondary",
      icon: <WarningCircleIcon className="text-[var(--color-alert-red)]" size={24} weight="regular" />
    },
    confirmIcon: {
      background: "bg-[var(--color-alert-night)] border border-[var(--color-alert-green)]",
      title: "text-[var(--color-alert-green)]",
      ...
      primary_button: "button-alert-green-primary",
      secondary_button: "button-alert-green-secondary",
      icon: <CheckCircleIcon className="text-[var(--color-alert-green)]" size={24} weight="regular"/>
    },
  }
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen} { ...props } >
      ...
      <AlertDialogContent className={cn(VARIANT_MAP[variant].background)}  onPointerDownOutside={() => setOpen(false)} onInteractOutside={() => setOpen(false)}>
          ...
          <div className="p-2 h-full flex gap-2">
            ...
            <div className="flex flex-col flex-1 gap-2">
              <div className="flex-1 flex">
                { <AlertDialogTitle className={VARIANT_MAP[variant].title} >{title}</AlertDialogTitle> }
              </div>
              <div className="flex-1 flex">
                { <AlertDialogDescription className={VARIANT_MAP[variant].text}>{description}</AlertDialogDescription> }
              </div>
            </div>
          </div>
        </AlertDialogHeader>
        ...
        <AlertDialogFooter> 
          {confirm_cancel && <AlertDialogCancel className={VARIANT_MAP[variant].secondary_button}>{cancelEdit}</AlertDialogCancel>}
          {confirm_cancel && <AlertDialogAction className={VARIANT_MAP[variant].primary_button}>{confirmEdit}</AlertDialogAction>}
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  )
